...
{
  // add value to back of heap
  values.push_back(value);

  int newIndex = values.size() - 1;
  int parentIndex = (newIndex - 1) / 2;

  // while values[newIndex] is bigger than values[parentIndex],
  // swap the two.
  while (parentIndex >= 0 && values[newIndex] > values[parentIndex])
  {
...
{
  T last_idx = values.size() - 1;
  // iterate through values until we find a value matching the parameter
  for (int i = 0; i < values.size(); i++)
  {
    if (values[i] == value)
    {
      std::swap(values[i], values[last_idx]);
      values.pop_back();
      last_idx--;
    }
  }

  // find index of largest element in values
  int max_idx = 0;
